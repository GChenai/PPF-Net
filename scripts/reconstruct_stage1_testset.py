...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Reconstruct the stage1 test set with the THz-only pixel-wise model.")
    parser.add_argument("--manifest", type=Path, default=Path("outputs/ppfnet_stage1/splits/test_pairs.csv"))
    parser.add_argument(
        "--checkpoint",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/ppfnet_stage1_pixel_fs/predictions/test_reconstruction"),
    )
...
def main() -> int:
    ...
    if normalization != "none":
        raise NotImplementedError(
            ...
            "Run the stage1 pixel model with --normalization none."
        )
    ...
    for row in manifest_rows:
        raw_csv_column = infer_raw_column(manifest_rows, modality)
        ...
        header_metadata["鎴愬儚妯″紡"] = "Stage1 THz-only閲嶅缓鍙嶅皠鍏夎氨"
