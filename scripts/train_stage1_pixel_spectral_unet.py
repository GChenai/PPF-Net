...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a pixel-wise spectral reconstruction U-Net.")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/ppfnet_stage1_pixel_fs"))
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional per-sample pixel cap to control dataset size.",
    )
...
def main() -> int:
    ...
    (logs_dir / "stage1_pixel_spectral_unet_config.json").write_text(
    ...
    metrics_path = logs_dir / "stage1_pixel_spectral_unet_metrics.csv"
    best_checkpoint_path = checkpoints_dir / "stage1_pixel_spectral_unet_best.pt"
    last_checkpoint_path = checkpoints_dir / "stage1_pixel_spectral_unet_last.pt"
    ...
    with metrics_path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        (logs_dir / "stage1_pixel_spectral_unet_test_metrics.json").write_text(
    ...
    (logs_dir / "stage1_pixel_spectral_unet_best_summary.json").write_text(
        json.dumps(json_safe_metrics(best_row), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    plot_spectral_training_curves(history_rows, logs_dir, prefix="stage1_pixel_spectral_unet")
