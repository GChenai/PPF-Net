...
LOG_PREFIX = "stage2_fs_only_baseline"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train an FS-only spectral reconstruction baseline on the Stage-2 paired RGB+FS split."
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/ppfnet_stage2_fs_only_baseline"))
    parser.add_argument(
        "--teacher-checkpoint",
        type=Path,
        default=None,
        help="Optional Stage-1 FS checkpoint used to initialize the FS-only baseline.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Per-sample pixel cap. Default matches the Stage-2 RGB-conditioned experiment.",
    )
...
def initialize_model_from_teacher_if_needed(
    model: SpectralUNet1D,
    ...
    teacher_base_channels = int(teacher_args.get("base_channels", args.base_channels))
    teacher_dropout = float(teacher_args.get("dropout", args.dropout))

    if teacher_base_channels != args.base_channels or abs(teacher_dropout - args.dropout) > 1e-8:
        raise ValueError(
            "Teacher checkpoint architecture mismatch. "
            "Expected base_channels={0}, dropout={1}, got base_channels={2}, dropout={3}. "
            "Match the training script arguments to the teacher checkpoint before initializing."
            .format(
    ...
    init_info["init_mode"] = "teacher_checkpoint"
    init_info["teacher_checkpoint"] = str(teacher_checkpoint_path)
    init_info["teacher_epoch"] = checkpoint.get("epoch")
    init_info["teacher_best_val_loss"] = checkpoint.get("best_val_loss")
    return init_info
...
def main() -> int:
    ...
    init_info = initialize_model_from_teacher_if_needed(model, args.teacher_checkpoint, args)

    data_info = {
        "task": "stage2_fs_only_baseline",
    ...
    print(
        "train_cubes={0} val_cubes={1} test_cubes={2} train_pixels={3} init_mode={4}".format(
    ...
    with metrics_path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            row = spectral_epoch_row(epoch, "stage2_fs_only", lr, train_metrics, val_metrics, elapsed)
