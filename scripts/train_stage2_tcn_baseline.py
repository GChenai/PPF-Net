...
LOG_PREFIX = "stage2_tcn_baseline"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a Stage-2 FS-only TCN spectral reconstruction baseline on the paired RGB+FS split."
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/ppfnet_stage2_tcn_baseline"))
...
def main() -> int:
    ...
    dilations = [2**idx for idx in range(max(args.num_blocks, 1))]
    model = SpectralTCN1D(
        in_channels=3,
    ...
    print(
        "train_cubes={0} val_cubes={1} test_cubes={2} train_pixels={3} model=tcn".format(
    ...
    with metrics_path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            row = spectral_epoch_row(epoch, "stage2_tcn", lr, train_metrics, val_metrics, elapsed)
