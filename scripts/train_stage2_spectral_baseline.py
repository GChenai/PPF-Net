...
from ppfnet.stage1_pixel_spectral_dataset import create_stage1_pixel_spectral_dataloaders
from ppfnet.stage1_spectral_unet import build_spectral_masked_inputs, spectral_reconstruction_loss
...
from ppfnet.stage2_spectral_baselines import build_stage2_spectral_baseline, supported_stage2_spectral_baselines


def default_output_dir_for_family(model_family: str) -> Path:
    return Path("outputs/ppfnet_stage2_{0}_baseline".format(model_family.lower()))


def log_prefix_for_family(model_family: str) -> str:
    return "stage2_{0}_baseline".format(model_family.lower())


def parse_args(default_model_family: str | None = None) -> argparse.Namespace:
    model_choices = list(supported_stage2_spectral_baselines())
    parser = argparse.ArgumentParser(
        description="Train a Stage-2 FS-only spectral baseline on the paired RGB+FS split."
    )
    parser.add_argument(
        "--model-family",
        choices=model_choices,
        default=default_model_family or "srcnn",
        help="Spectral baseline family to train.",
    )
    ...
    parser.add_argument("--srcnn-bottleneck-channels", type=int, default=32)
    parser.add_argument("--edsr-res-scale", type=float, default=0.1)
    ...
    if args.output_dir is None:
        args.output_dir = default_output_dir_for_family(args.model_family)
    return args
...
def run_model_step(model, batch: Dict[str, object], args: argparse.Namespace, generator: torch.Generator | None = None):
...
def train_one_epoch(model, dataloader, optimizer, scaler, device, epoch, args) -> Dict[str, float]:
    model.train()
...
@torch.no_grad()
def evaluate(model, dataloader, device, args, seed: int) -> Dict[str, float]:
    model.eval()
    running: Dict[str, float] = {}
    step_count = 0
...
def save_checkpoint(path, model, optimizer, scheduler, epoch, best_val_loss, args, data_info) -> None:
...
def main(default_model_family: str | None = None) -> int:
    args = parse_args(default_model_family=default_model_family)
    set_seed(args.seed)
    ...
    log_prefix = log_prefix_for_family(args.model_family)
    task_name = log_prefix

    (logs_dir / "{0}_config.json".format(log_prefix)).write_text(
        json.dumps(json_ready_args(args), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    dataloaders = create_stage1_pixel_spectral_dataloaders(
    ...
    model = build_stage2_spectral_baseline(
        ...
        srcnn_bottleneck_channels=args.srcnn_bottleneck_channels,
        edsr_res_scale=args.edsr_res_scale,
    ).to(device)
    ...
    data_info = {
        ...
        "normalization": args.normalization,
        "max_pixels_per_sample": args.max_pixels_per_sample,
        "train_num_pixel_spectra": len(train_dataset),
        "val_num_pixel_spectra": len(val_dataset),
        "test_num_pixel_spectra": len(test_dataset),
        "train_num_cubes": len(train_dataset.samples),
        "val_num_cubes": len(val_dataset.samples),
        "test_num_cubes": len(test_dataset.samples),
        "kernel_size": args.kernel_size,
        "num_blocks": args.num_blocks,
        "base_channels": args.base_channels,
        "srcnn_bottleneck_channels": args.srcnn_bottleneck_channels,
    ...
    print(
        "train_cubes={0} val_cubes={1} test_cubes={2} train_pixels={3} model={4}".format(
    ...
    metrics_path = logs_dir / "{0}_metrics.csv".format(log_prefix)
    best_checkpoint_path = checkpoints_dir / "{0}_best.pt".format(log_prefix)
    last_checkpoint_path = checkpoints_dir / "{0}_last.pt".format(log_prefix)
    ...
    with metrics_path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            row = spectral_epoch_row(epoch, task_name, lr, train_metrics, val_metrics, elapsed)
            history_rows.append(row)
            writer.writerow(row)
            handle.flush()

            print(
                "epoch {0}/{1} lr={2:.6f} train_loss={3:.4f} val_loss={4:.4f} val_psnr={5:.2f} elapsed={6:.1f}s".format(
                    epoch,
                    args.epochs,
                    lr,
                    train_metrics["loss"],
            ...
            save_checkpoint(last_checkpoint_path, model, optimizer, scheduler, epoch, best_val_loss, args, data_info)

            improved, should_stop = early_stopper.step(val_metrics["loss"], epoch)
            if improved:
                best_val_loss = float(val_metrics["loss"])
                best_epoch_row = dict(row)
                save_checkpoint(best_checkpoint_path, model, optimizer, scheduler, epoch, best_val_loss, args, data_info)
        ...
        (logs_dir / "{0}_test_metrics.json".format(log_prefix)).write_text(
    ...
    (logs_dir / "{0}_best_summary.json".format(log_prefix)).write_text(
        json.dumps(json_safe_metrics(best_row), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    plot_spectral_training_curves(history_rows, logs_dir, prefix=log_prefix)
